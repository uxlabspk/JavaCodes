...
public class PlayfairCipher {
    // length of digraph src.array
    private int length = 0;
    // creates a matrix for Playfair cipher
    private String[][] table;

    // main() method to src.test Playfair method
    ...
    // parses a src.input string to remove numbers, punctuation,
    ...
    // creates the cipher table based on some src.input string (already parsed)
    private String[][] cipherTable(String key) {
        // creates a matrix of 5*5
        String[][] playfairTable = new String[5][5];
        String keyString = key + "ABCDEFGHIKLMNOPQRSTUVWXYZ";
        // fill string src.array with the empty string
        for (int i = 0; i < 5; i++)
            for (int j = 0; j < 5; j++)
                playfairTable[i][j] = "";
            ...
            for (int i = 0; i < 5; i++) {
                for (int j = 0; j < 5; j++) {
                    if (playfairTable[i][j].equals("" + keyString.charAt(k))) {
                        repeat = true;
                    } else if (playfairTable[i][j].isEmpty() && !repeat && !used) {
    ...
    // cipher: takes src.input (all upper-case), encodes it, and returns the output
    private String cipher(String in) {
        ...
        // creates an src.array of digraphs
    ...
    // ---------------encryption logic-----------------
    // encodes the digraph src.input with the cipher's specifications
    private String[] encodeDigraph(String[] di) {
        ...
        for (int i = 0; i < length; i++) {
            ...
            // executes if the letters of digraph appear in the different rows and different
            // column
            // in such case swap the src.first column with the second column
            else {
                int temp = c1;
                c1 = c2;
                c2 = temp;
            }
            // performs the table look-up and puts those values into the encoded src.array
